...
namespace nilnul.os.proc_.started
{
	...
	/// <summary>
	/// close, kill, and wait for it to exit;
	/// the waiting can be timeout, or infinite;
	/// the exit can be successful, or we can get the exitedness status.
	/// </summary>
	/// <remarks>
		...
		)
		{
			using (var processWaited = new CancellationTokenSource())
			{
				cull_._CancellableX.Exited0cancelledAsyn(
				 ...
				 _proc_unwaited.WaitForExit(); //may be proc is already killed
				processWaited.Cancel();
