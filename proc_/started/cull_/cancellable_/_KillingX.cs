...
namespace nilnul.os.proc_.started.quit_._abort
{
...
In the case of the task manager, the exit code is set to 1, but I don't know if it's documented anywhere.
		...
		)
		{
			...
			var t= await close_._DeferX.Exited0cancelledAsyn(
				_proc_started
			, processWaited
			, minimum4user2close
			...
			return proc_.started.kill_._Exit_someXpnAsExitedX.Exited(_proc_started);
			try
			{
